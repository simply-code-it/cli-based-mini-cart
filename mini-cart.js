...
class Cart {
  ...
  viewCart() {
    ...
    this.items.forEach((item) => {
      ...
      console.log(
        `${item.product.name} - Quantity: ${
          item.quantity
        }, Price: ${item.product.price.toFixed(2)} USD, Total: ${total.toFixed(
          2
        )} USD`
      );
    });
    ...
    console.log(
      `Total (before discounts): ${totalBeforeDiscount.toFixed(2)} USD`
    );
  }
    ...
    this.items.forEach((item) => {
      ...
      if (item.product.category === "Fashion" && item.quantity >= 2) {
        ...
        console.log(
          `Buy 1 Get 1 Free on ${
            item.product.name
          } applied. Discount: ${discount.toFixed(2)} USD`
        );
      }
        ...
        (answer) => {
          if (answer.toLowerCase() === "yes") {
            rl.question("Enter currency (USD, EUR, GBP): ", (currency) => {
              ...
              console.log(
                `Final Total in ${currency}: ${convertedTotal.toFixed(
                  2
                )} ${currency}`
              );
              rl.prompt();
